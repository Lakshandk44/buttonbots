...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
    ...
    conn.on('qr', async qr => {
      let scan = await parent.sendFile(m.chat, await qrcode.toDataURL(qr, { scale: 8 }), 'qrcode.png', 'මෙම qr කේතය \n\n1.වට්ස් ඇප් වෙබ්හි ස්කෑන් කරන්න\n2. ඔබත් බොට් කෙනෙකු වේn3.  \nQR Expired 20 seconds', m)
      setTimeout(() => {
